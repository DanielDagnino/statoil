...
# #******************************************************************************#
from nn_utils import image_rot, set_mean_and_norm, image_shift, image_perturbation_over, image_mirror, image_corr   #@UnusedImport
...
#******************************************************************************#
# 
def read_data_set_BN_augm(augm, ind_read_set, file_name, parameters_min, calc_BN, mean_BN=None, std_BN=None):
	...
	shift, pertub, mirror, rotate, normalize = parameters_min
	
	# Shuffle.
	ne_augm = augm*ne
# 	permutation = list(np.random.permutation(augm*ind_read_set))
	permutation = list(np.random.permutation(np.repeat(np.array(ind_read_set),augm)))
# 	permutation = range(len(ind_read_set))
	
	#------------------------------------------------------------------------------#
	#------------------------------------------------------------------------------#
	# Read.
	y = np.zeros((ne_augm,1),dtype="uint16")
	e = np.full((ne_augm,1), True, dtype=bool)
	file_id = ["" for _ in range(ne_augm)]
	x = np.zeros((ne_augm,D,H,W),dtype="float64")
	imag = np.zeros((H,W,D),dtype="float64")
	for kk in range(ne_augm):
		k = permutation[kk]
		...
		if correlation:
			imag[:,:,0] = image_corr( imag[:,:,0] )
			imag[:,:,1] = image_corr( imag[:,:,1] )
		
		if rotate:	imag = image_rot( imag )
...
# 		if normalize:	imag, _, _ = set_mean_and_norm( imag )
	...
	if calc_BN:
		...
		#------------------------------------------------------------------------------#
		mean_BN /= count
		std_BN = ((count-1)/count)*np.sqrt(std_BN/count - np.power(mean_BN,2))
